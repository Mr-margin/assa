...
public class SH3_Controller extends MultiActionController{
	...
	public  ModelAndView H3_pie_All(HttpServletRequest request,HttpServletResponse response) throws IOException{
		...
		if (mokuai_name.equals("jiatingshouzhi")) {	//根据模块名称来判断执行模块 家庭收支模块
			...
			int wys = 0; //5000元以上
			String pkid_sql = "SELECT pkid,v9 FROM da_household"+year+" where  sys_standard='"+ leixing_name +"'"+shilevel_sql+"  and  v21!='已脱贫'";
...
//			}
			...
			sql="select nnum,count(*)as count from (select substring(v8,7,4) as nnum from da_household"+year+" where (CHAR_LENGTH(v8)=18 OR CHAR_LENGTH(v8)=20)"+shilevel_sql+"AND sys_standard='"+leixing_name+"'  AND  v21!='已脱贫' UNION ALL select substring(y.v8,7,4) as nnum  from da_household"+year+" x join da_member"+year+" y on x.pkid=y.da_household_id where x.sys_standard='"+leixing_name+"'  AND x. v21!='已脱贫' and (CHAR_LENGTH(y.v8)=18 OR CHAR_LENGTH(y.v8)=20) "+XZQH+") x group by nnum";
			SQLAdapter sqlAdapter =new SQLAdapter(sql);
			List<Map> sql_list = this.getBySqlMapper.findRecords(sqlAdapter);
			JSONObject val = new JSONObject();
			if(sql_list.size()>0){
...
//			}
			...
			if (shilevel==1) {
				sql="SELECT v3,COUNT(v9) AS count FROM da_household"+year+" WHERE v9='"+shujv+"' and v21!='已脱贫' and sys_standard='"+leixing_name+"' and v2 like '%"+shi_name+"%' GROUP BY v3";
			}else if (shilevel==2) {
				sql="SELECT v4 as v3,COUNT(v9) AS count FROM da_household"+year+" WHERE v9='"+shujv+"'and v21!='已脱贫' and sys_standard='"+leixing_name+"' and v3 like '%"+ shi_name +"%' GROUP BY v4";	
			}else if (shilevel==3) {
				sql="SELECT v5 as v3 ,COUNT(v9) AS count FROM da_household"+year+" WHERE v9='"+shujv+"'and v21!='已脱贫' and sys_standard='"+leixing_name+"' and v4 like '%"+ shi_name +"%' GROUP BY v5";
			}else if (shilevel==4) {
				sql="SELECT v5 as v3 ,COUNT(v9) AS count FROM da_household"+year+" WHERE v9='"+shujv+"'and v21!='已脱贫' and sys_standard='"+leixing_name+"' and v5 like '%"+ shi_name +"%' GROUP BY v5";
			}
			SQLAdapter sqlAdapter =new SQLAdapter(sql);
			List<Map> sql_list = this.getBySqlMapper.findRecords(sqlAdapter);
			...
			if(shilevel==1){//当层级为1的时候
				sql="SELECT b2.v3,COUNT(*)as count FROM("
						...
						+ ")b1 INNER JOIN da_household"+year+" b2 ON b1.da_household_id = b2.pkid WHERE b2.sys_standard='"+leixing_name+"'  and b2. v21!='已脱贫' and b2.v2='鄂尔多斯市' GROUP BY b2.v3";
			}else if(shilevel==2){//当层级为2的时候
				sql="SELECT b2.v4 as v3,COUNT(*)as count FROM("
						+ "SELECT a2.* FROM sys_personal"+year+" a1 INNER JOIN sys_personal_household_many"+year+" a2 ON  a1.pkid=a2.sys_personal_id "
						+ ")b1 INNER JOIN da_household"+year+" b2 ON b1.da_household_id = b2.pkid  WHERE b2.sys_standard='"+leixing_name+"'  and b2. v21!='已脱贫' AND b2.v3='"+shi_name+"' GROUP BY b2.v4";
			}else if(shilevel==3){//当层级为3的时候
				sql="SELECT b2.v5 as v3,COUNT(*)as count FROM("
						+ "SELECT a2.* FROM sys_personal"+year+" a1 INNER JOIN sys_personal_household_many"+year+" a2 ON  a1.pkid=a2.sys_personal_id "
						+ ")b1 INNER JOIN da_household"+year+" b2 ON b1.da_household_id = b2.pkid  WHERE b2.sys_standard='"+leixing_name+"'  and b2. v21!='已脱贫' AND b2.v4='"+shi_name+"' GROUP BY b2.v5";
			}else if(shilevel==4){//当层级为4的时候
				sql="SELECT b2.v5 as v3,COUNT(*)as count FROM("
						+ "SELECT a2.* FROM sys_personal"+year+" a1 INNER JOIN sys_personal_household_many"+year+" a2 ON  a1.pkid=a2.sys_personal_id "
						+ ")b1 INNER JOIN da_household"+year+" b2 ON b1.da_household_id = b2.pkid  WHERE b2.sys_standard='"+leixing_name+"'  and b2. v21!='已脱贫' AND b2.v5='"+shi_name+"' GROUP BY b2.v5";
			}
			SQLAdapter sqlAdapter =new SQLAdapter(sql);
			List<Map> sql_list = this.getBySqlMapper.findRecords(sqlAdapter);
			JSONArray jsa=new JSONArray();
