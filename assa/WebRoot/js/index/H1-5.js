$(function () {
	...
	$("#zoufangqingkaung #v1_tj_zoufang").click(function () {
		if($("#zoufangqingkaung #v1").val()!=''&&$("#zoufangqingkaung #v2").val()!=''&&$("#zoufangqingkaung #v3").val()!=''){
			$.ajax({  		       
			    url: "/assa/getSave_cuoshi.do",
			    type: "POST",
			    async:false,
			    dataType: "text",
			    data:{
			    	pkid: $("#shang_yi #hu_pkid").val(),
			    	v1 : $("#zoufangqingkaung #v1").val(),
			    	v2 : $("#zoufangqingkaung #v2").val(),
			    	v3 : $("#zoufangqingkaung #v3").val(),
			    	type : 2
		        },
	...
	$("#zoufangqingkaung #v1_bc_zoufang").click(function () {
		$.ajax({  		       
		    url: "/assa/getUpdate_cuoshi.do",
		    type: "POST",
		    async:false,
		    dataType: "text",
		    data:{
		    	pkid: $("#zoufangqingkaung #cuoshi_pkid").val(),
		    	v1 : $("#zoufangqingkaung #v1").val(),
		    	v2 : $("#zoufangqingkaung #v2").val(),
		    	v3 : $("#zoufangqingkaung #v3").val(),
		    	type : 2
...
//	    		if(item.pic_num==0){
//	    			cuoshi_html += "<td class=\"client-status\"><button type=\"button\" class=\"btn btn-primary btn-xs\" onclick='zoufang_pic_load(1,"+item.pkid+");'>上传</button></td>";
//	    		}else{
