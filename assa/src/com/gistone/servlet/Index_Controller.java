		...
		if(session.getAttribute("Login_map")!=null){//验证session不为空
			...
			if(code.toString().equals("shi")==true){//市级用户
				...
				String sql="select * from  (select * from (select com_name  from sys_company s where s.com_f_pkid = "+pkid+" )a  LEFT JOIN  ( SELECT         b.v3,          count,          count1,          sum,          sum1       FROM          (             SELECT                v3,          "
						...
						+ "     RIGHT JOIN(          SELECT             v3,             count(*)AS count1,             sum(v9)AS sum1          FROM             da_household_2016          WHERE             sys_standard = '"+gors+"'          AND v21 != '已脱贫'     AND entry_year = '2016'     GROUP BY             v3       )b ON a.v3 = b.v3  "
						...
						+ "     sys_standard = '"+gors+"'          AND v21 != '已脱贫'     AND entry_year = '2016'    GROUP BY             v3       )b ON a.v3 = b.v3  )d on a.com_name = d.v3 )e INNER JOIN ( select * from (select com_name from sys_company s where s.com_f_pkid = "+pkid+")a  LEFT JOIN  (  SELECT       a.v3,       sum2,       sum3    FROM       (          SELECT         "
...
//				String sql="SELECT y1.v4 AS v3,count(*) AS count,sum(y1.v9) AS sum FROM da_household y1 JOIN sys_company y2 ON y1.v3 = y2.com_name where y2.com_code="+code+" AND y1.sys_standard='"+gors+"' and v21!='已脱贫'  group by y1.v4";
				...
				String sql="select * from  (select * from (select com_name from sys_company s where s.com_f_pkid ="+pkid+" )a  LEFT JOIN  ( SELECT          b.v3,          count,          count1,          sum,          sum1       FROM          (             SELECT              "
						...
						+ "     JOIN sys_company y2 ON y1.v3 = y2.com_name          WHERE             y2.com_code = "+code+"          and             sys_standard = '"+gors+"'          AND v21 != '已脱贫'    AND entry_year = '2016'      GROUP BY             y1.v4       )b ON a.v3 = b.v3   UNION  SELECT          a.v3,          count,  "
						...
						+ "  count(*)AS count1,             sum(v9)AS sum1          FROM             da_household_2016 y1          JOIN sys_company y2 ON y1.v3 = y2.com_name          WHERE             y2.com_code = "+code+" 					and             sys_standard = '"+gors+"'          AND v21 != '已脱贫'  AND entry_year = '2016'  "
...
/*		2017-4-26之前更改		String sql = "SELECT   	*    FROM   	(   		SELECT   			a.v3,   			count,   			count1,   			sum,   			sum1   		FROM   			(   				SELECT   	"
				...
				String sql =" select * from  ( select * from (select com_name from sys_company s where s.com_f_pkid ="+pkid+" )a  LEFT JOIN  ( SELECT          b.v3,          count,          count1,          sum,          sum1       FROM          (             SELECT                y1.v5 as v3,      "
						...
						+ "     JOIN sys_company y2 ON y1.v4 = y2.com_name          WHERE             y2.com_code = '"+code+"'          and             sys_standard = '"+gors+"'          AND v21 != '已脱贫'  AND entry_year = '2016'        GROUP BY             y1.v5       )b ON a.v3 = b.v3   UNION  SELECT          a.v3,      "
						...
						+ "     count(*)AS count1,             sum(v9)AS sum1          FROM             da_household_2016 y1          JOIN sys_company y2 ON y1.v4 = y2.com_name          WHERE             y2.com_code = '"+code+"' 					and             sys_standard = '"+gors+"'          AND v21 != '已脱贫'   AND entry_year = '2016'  "
