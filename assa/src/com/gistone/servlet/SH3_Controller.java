...
public class SH3_Controller extends MultiActionController{
		...
		if (mokuai_name.equals("jiatingshouzhi")) {	//根据模块名称来判断执行模块 家庭收支模块
			...
			if (com_level.equals("1")) { //用户层级为1时
				if (shujv.equals("level-1")) {
					sql = "SELECT ROUND(v24,2) as num FROM da_household where sys_standard='"+ leixing_name +"'";
				}else {
					sql = "SELECT ROUND(v24,2) as num FROM da_household where v3='"+shujv+"' and sys_standard='"+ leixing_name +"'";
				}
			}else if (com_level.equals("2")) {	//层级为2的用户
				if (shujv.equals("level-1")) {
					sql = "SELECT ROUND(v24,2) as num FROM da_household where v3='"+ com_name +"' and sys_standard='"+ leixing_name +"'";
				}else {
					sql = "SELECT ROUND(v24,2) as num FROM da_household where v3='"+shujv+"' and sys_standard='"+ leixing_name +"'";
				}
			}else if (com_level.equals("3")) {	//层级为3的时候用户的名字
				if (shujv.equals("level-1")) {
					 sql = "SELECT ROUND(v24,2) as num FROM da_household where v4='"+ com_name +"' and sys_standard='"+ leixing_name +"'";
				}else {
					sql = "SELECT ROUND(v24,2) as num FROM da_household where v4='"+shujv+"' and sys_standard='"+ leixing_name +"'";
				}
			}else {		//获取上级名称
			...
			if (com_level.equals("1")) {	//当用户层级为1时
				if (shujv.equals("level-1")) {
					sql="select SUM(b2) as b2,SUM(b12) as b12,(SUM(b2)-SUM(b12)) as b3 from sys_company t1"+  
								  " left join (select v3,COUNT(*) as b2,sum(v9) as b3 from da_household where sys_standard='"+ leixing_name +"'  group by v3) t2 on t1.com_name=t2.v3"+  
								  " left join (select v3,COUNT(*) as b12 from b12_t where sys_standard='"+ leixing_name +"'  group by v3) t12 on t1.com_name=t12.v3"+
					...
					sql="select SUM(b2) as b2,SUM(b12) as b12,(SUM(b2)-SUM(b12)) as b3,t2.v3 from sys_company t1"+ 
							  " left join (select v3,v4,COUNT(*) as b2,sum(v9) as b3 from da_household where sys_standard='"+ leixing_name +"'  group by v3) t2 on t1.com_name=t2.v3"+ 
							  " left join (select v3,v4,COUNT(*) as b12 from b12_t where sys_standard='"+ leixing_name +"'  group by v3) t12 on t1.com_name=t12.v3 "+
					...
					sql="select SUM(b2) as b2,SUM(b12) as b12,(SUM(b2)-SUM(b12)) as b3,t2.v4 from sys_company t1"+ 
						  " left join (select v3,v4,COUNT(*) as b2,sum(v9) as b3 from da_household where sys_standard='"+ leixing_name +"'  group by v4) t2 on t1.com_name=t2.v3"+ 
						  " left join (select v3,v4,COUNT(*) as b12 from b12_t where sys_standard='"+ leixing_name +"'  group by v4) t12 on t1.com_name=t12.v3 "+
			...
			sql = "SELECT SUM(v9)as count,v23 as v3 FROM da_household WHERE sys_standard='"+leixing_name+"' GROUP BY v23";
			...
			if(com_level.equals("1")){
				if(shujv.equals("level-1")){
					sql="select nnum,count(*)as count from (select substring(v8,7,4) as nnum from da_household where (CHAR_LENGTH(v8)=18 OR CHAR_LENGTH(v8)=20) AND sys_standard='"+leixing_name+"' UNION ALL select substring(y.v8,7,4) as nnum  from da_household x join da_member y on x.pkid=y.da_household_id where x.sys_standard='"+leixing_name+"' and (CHAR_LENGTH(y.v8)=18 OR CHAR_LENGTH(y.v8)=20) ) x group by nnum";
				}else{
					sql="select nnum,count(*)as count from (select substring(v8,7,4) as nnum from da_household where (CHAR_LENGTH(v8)=18 OR CHAR_LENGTH(v8)=20)and v3='"+shujv+"'AND sys_standard='"+leixing_name+"'UNION ALL select substring(y.v8,7,4) as nnum  from da_household x join da_member y on x.pkid=y.da_household_id where x.sys_standard='"+leixing_name+"' and (CHAR_LENGTH(y.v8)=18 OR CHAR_LENGTH(y.v8)=20) AND  y.v3='"+shujv+"') x group by nnum";
				}
				...
				if(shujv.equals("level-1")){
					...
					sql="select nnum,count(*)as count from (select substring(v8,7,4) as nnum from da_household where (CHAR_LENGTH(v8)=18 OR CHAR_LENGTH(v8)=20)and v4='"+shujv+"'AND sys_standard='"+leixing_name+"'UNION ALL select substring(y.v8,7,4) as nnum  from da_household x join da_member y on x.pkid=y.da_household_id where x.sys_standard='"+leixing_name+"' and (CHAR_LENGTH(y.v8)=18 OR CHAR_LENGTH(y.v8)=20) AND  y.v4='"+shujv+"') x group by nnum";
				}
			...
			if (com_level.equals("1")) {
				if(com_level.equals("1")==true){	//市级用户
					if(shujv.equals("level-1")==true){
						sql="SELECT v9,COUNT(v9) as count FROM da_household where sys_standard='"+leixing_name+"'GROUP BY v9";
					}else {
						sql="SELECT v9,COUNT(v9) as count FROM da_household WHERE v3='"+shujv+"' and sys_standard='"+leixing_name+"' GROUP BY v9";
					}
			}
			}else if (com_level.equals("2")) {
				sql="SELECT v9,COUNT(v9) as count FROM da_household where v3='"+com_name+"' and sys_standard='"+leixing_name+"'GROUP BY v9";
			}else if (com_level.equals("3")) {
				sql="SELECT v9,COUNT(v9) as count FROM da_household where sys_standard='"+leixing_name+"' and v4='"+com_name+"' GROUP BY v9";
			}else {
			com_name=company_json.get("xiang").toString();//获取上级用户名称
				sql="SELECT v9,COUNT(v9) as count FROM da_household where sys_standard='"+leixing_name+"' and v4='"+com_name+"' GROUP BY v9";
			}
	...
	public ModelAndView H3_map_All(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (mokuai_name.equals("jiatingshouzhi")) {	//根据模块名称来判断执行模块 家庭收支模块
			...
			if (com_level.equals("1")) {	//当用户层级为1时
				sql="select v3,count(*) as count from (SELECT ROUND(v24,2) as num ,v3 FROM da_household where  sys_standard='"+ leixing_name +"') h where "+tiaojian+" group by v3";
			}else if (com_level.equals("2")) {	//当用户层级为2时
				sql="select v3,count(*) as count from (SELECT ROUND(v24,2) as num , v4 as v3 FROM da_household where v3='"+ com_name +"' and v3= sys_standard='"+ leixing_name +"') h where "+tiaojian+" group by v3";
			}else if (com_level.equals("3")) {	//当用户层级为3时
				sql="select v3,count(*) as count from (SELECT ROUND(v24,2) as num , v5 as v3 FROM da_household where v4='"+ com_name +"' and v3= sys_standard='"+ leixing_name +"') h where "+tiaojian+" group by v3";
			}else {	//当用户层级为4时
				com_name=company_json.get("xiang").toString();	//获取上级用户
				sql="select v3,count(*) as count from (SELECT ROUND(v24,2) as num , v5 as v3 FROM da_household where v4='"+ com_name +"' and v3= sys_standard='"+ leixing_name +"') h where "+tiaojian+" group by v3";
			}
			...
			if (com_level.equals("1")) {	//当层级为1时
				...
				sql="select b2,b12,t2.v4 as b1 ,(b2-b12) as v3 from sys_company t1  "+
						   "left join (select v4,v3,COUNT(*) as b2,sum(v9) as b3 from da_household where sys_standard='"+ leixing_name +"'  group by v4) t2 on t1.com_name=t2.v3 "+
						   "left join (select v3,v4,COUNT(*) as b12 from b12_t where sys_standard='"+ leixing_name +"'  GROUP BY v4  ) t12 on t1.com_name=t12.v3  "+
				...
				sql="select b2,b12,t2.v5 as b1,(b2-b12) as v3 from sys_company t1  "+
						   "left join (select  v4,v5,v3,COUNT(*) as b2,sum(v9) as b3 from da_household where sys_standard='"+ leixing_name +"'  group by v5) t2 on t1.com_name=t2.v3  "+
						   "left join (select v3,v4,v5,COUNT(*) as b12 from b12_t where sys_standard='"+ leixing_name +"'  GROUP BY v5  ) t12 on t1.com_name=t12.v3  "+
			...
			if (com_level.equals("1")==true) {
				sql="SELECT v3,SUM(v9)AS count FROM da_household WHERE sys_standard='"+leixing_name+"' AND v23 LIKE '%"+shujv+"%' GROUP BY v3";
			}else if (com_level.equals("2")==true) {
				sql="SELECT v4 as v3,SUM(v9)AS count FROM da_household WHERE sys_standard='"+leixing_name+"' AND v23 LIKE '%"+shujv+"%' and v3='"+com_name+"' GROUP BY v4";
			}else if (com_level.equals("3")==true) {
				sql="SELECT v5 as v3,SUM(v9)AS count FROM da_household WHERE sys_standard='"+leixing_name+"' AND v23 LIKE '%"+shujv+"%' and v4='"+com_name+"' GROUP BY v5";
			}else {
				com_name=company_json.get("xiang").toString();	//获取上级用户
				sql="SELECT v5 as v3,SUM(v9)AS count FROM da_household WHERE sys_standard='"+leixing_name+"' AND v23 LIKE '%"+shujv+"%' and v4='"+com_name+"' GROUP BY v5";
			}
			...
			if (com_level.equals("1")) {
				sql="SELECT v3,COUNT(v9) AS count FROM da_household WHERE v9='"+shujv+"'and sys_standard='"+leixing_name+"' GROUP BY v3";
			}else if (com_level.equals("2")) {
				sql="SELECT v4 as v3,COUNT(v9) AS count FROM da_household WHERE v9='"+shujv+"'and sys_standard='"+leixing_name+"' and v3='"+com_name+"' GROUP BY v4";	
			}else if (com_level.equals("3")) {
				sql="SELECT v5 as v3 ,COUNT(v9) AS count FROM da_household WHERE v9='"+shujv+"'and sys_standard='"+leixing_name+"' and v4='"+com_name+"' GROUP BY v5";
			}else {
				com_name=company_json.get("xiang").toString();//获取上级用户名称
				sql="SELECT v5 as v3 ,COUNT(v9) AS count FROM da_household WHERE v9='"+shujv+"'and sys_standard='"+leixing_name+"' and v4='"+com_name+"' GROUP BY v5";
			}
			...
			if(com_level.equals("1")){//当层级为1的时候
				sql="SELECT b2.v3,COUNT(*)as count FROM("
						+ "SELECT a2.* FROM sys_personal a1 INNER JOIN sys_personal_household_many a2 ON  a1.pkid=a2.sys_personal_id "
						+ ")b1 INNER JOIN da_household b2 ON b1.da_household_id = b2.pkid WHERE b2.sys_standard='"+leixing_name+"' GROUP BY b2.v3";
			}else if(com_level.equals("2")){//当层级为2的时候
				sql="SELECT b2.v4 as v3,COUNT(*)as count FROM("
						+ "SELECT a2.* FROM sys_personal a1 INNER JOIN sys_personal_household_many a2 ON  a1.pkid=a2.sys_personal_id "
						+ ")b1 INNER JOIN da_household b2 ON b1.da_household_id = b2.pkid  WHERE b2.sys_standard='"+leixing_name+"' AND b2.v3='"+com_name+"' GROUP BY b2.v4";
			}else if(com_level.equals("3")){//当层级为3的时候
				sql="SELECT b2.v5 as v3,COUNT(*)as count FROM("
						+ "SELECT a2.* FROM sys_personal a1 INNER JOIN sys_personal_household_many a2 ON  a1.pkid=a2.sys_personal_id "
						+ ")b1 INNER JOIN da_household b2 ON b1.da_household_id = b2.pkid  WHERE b2.sys_standard='"+leixing_name+"' AND b2.v4='"+com_name+"' GROUP BY b2.v5";
			...
			if(com_level.equals("1")){//当层级为1的时候
				sql="SELECT d1.count,d2.com_name FROM ("
						...
						+ "SELECT * FROM (SELECT v5,sys_company_id from  da_company WHERE v5 IS NOT NULL"
				...
				sql="SELECT d2.com_name,d1.count FROM ("
						...
						+ "SELECT * FROM ("
						+ "SELECT v5 from  da_company WHERE v5 IS NOT NULL"
				...
				sql="SELECT * FROM ("
						+ "SELECT v5 from  da_company WHERE v5 IS NOT NULL)a1 INNER JOIN sys_company a2 ON (a1.v5=a2.pkid)OR(a1.v5=a2.com_name) "
